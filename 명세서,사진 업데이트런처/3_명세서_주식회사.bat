@echo off
chcp 65001 >nul
REM === 명세서 추출(주식회사) — seek 자동 + 마지막5 표시 + 테넷 fast ===
set "RAW=C:\Users\USER\Desktop\hdsign\easyform-data\easyform_2026_corp_fast.json"
set "CSV=C:\Users\USER\Desktop\hdsign\easyform-data\26년도매출거래목록(주식회사).csv"
set "ENR=C:\Users\USER\Desktop\hdsign\auto-quote-data\invoices\easyform_2026_corp.json"
...
echo ============== 명세서 추출 (주식회사) ==============
echo.
echo  [0단계] 이지폼에서 26년 주식회사 매출거래목록을 [엑셀(csv)] 로 내보내기.
echo    * 저장 폴더 (여기에 덮어쓰기):
echo        C:\Users\USER\Desktop\hdsign\easyform-data\
echo    * 파일 이름 (정확히 이 이름으로):
echo        26년도매출거래목록(주식회사).csv
echo    * !!! 주의: 'auto-quote-data\invoices' 폴더에는 절대 넣지 마세요 !!!
echo          (거기는 프로그램이 자동으로 만드는 곳 - CSV 넣으면 데이터 깨짐)
echo.
py -3 "C:\Users\USER\Desktop\hdsign\scripts\easyform_csv_check.py" corp
echo.
echo CSV 새로 했으면 아무 키. 이지폼 주식회사 목록 정순/최대화/1번째 행 클릭(Enter 금지) 해두세요.
pause >nul

echo.
for /f %%n in ('py -3 "C:\Users\USER\Desktop\hdsign\scripts\easyform_n.py" corp') do set N=%%n
echo 목록을 아래로 %N% 칸 자동으로 내립니다. 키 누른 즉시 '이지폼 창'을 클릭해 활성 상태로 두세요.
pause >nul
py -3 "%SEEK%" %N%

echo.
py -3 "C:\Users\USER\Desktop\hdsign\scripts\easyform_last.py" corp 5
echo.
