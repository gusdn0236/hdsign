...
### 순서(오래된순/최신순)는 상관없음 — 안 꼬인다
3중 안전: ① `seen` ID 스킵 ② `_sort_by_date.py` 가 각 사진을 **자기 createdAt** 으로 이름지음(다운로드 순서 무관)
③ `_dedup_samekey.py` 가 `_i_<hex>` 내용키로 중복 제거. → 그리드 정렬은 아무거나 둬도 됨.
...
## D. 매칭 (사진 ↔ 명세서, N:N)
```
py -3 hdsign/auto-quote-data/learning/match_photos_to_invoices_v4.py
```
- 거래처+[발주~출고+7] 창, 품목코드·규격 게이트, 연도=명세서로 확정.
- 산출: `match_links.json` / `invoice_links.json`.

## E. 병합 + R2 업로드 (신규 증분만)
기존 매칭 유지 + 신규 (file,idx) 만 `photo_merge_new.json` 생성 → R2 업로드:
```
# (병합 맵 생성 = match_photos_to_invoices_v4 결과로 photo_merge_new.json 빌드)
