...
const Departments = () => {
    ...
    return (
        <div className="departments-page">
            ...
            <p className="departments-subtitle">에이치디사인은 층별로 전문화된 공간에서 최고의 품질을 만들어냅니다.</p>
            ...
            <div className="departments-layout">
                ...
                {/* 오른쪽 상세 내용 */}
                <div className="floor-detail" style={{ borderTopColor: floors[activeFloor].color }}>
                    <div className="floor-detail-header">
                        <span className="floor-detail-floor" style={{ color: floors[activeFloor].color }}>
                            {floors[activeFloor].floor}
                        </span>
                        <h3 className="floor-detail-name">{floors[activeFloor].name}</h3>
                        <p className="floor-detail-desc">{floors[activeFloor].description}</p>
                    </div>
                    <ul className="floor-detail-list">
                        {floors[activeFloor].details.map((item, idx) => (
                            <li key={idx} style={{ borderLeftColor: floors[activeFloor].color }}>
                                {item}
                            </li>
                        ))}
                    </ul>
