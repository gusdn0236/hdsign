 ...
 *
 * ──────────────────────────────────────────────────────────────────────────────
 *  사무실 단가표 형식의 특징 (사용자 워크플로우 — 절대 잊지 말 것)
 * ──────────────────────────────────────────────────────────────────────────────
 *
 *  1) 가격 시프트 갱신
 *     사무실에서 단가표를 수정할 때, **작은 사이즈 가격을 한 칸 지우고 큰 사이즈
 *     가격을 한 칸씩 위로 땡겨오면서** 약간씩 수정한다. 결과적으로 한 컬럼의
 *     마지막(가장 큰) 사이즈는 빈칸이 된다 (가격이 한 칸 위로 갔으니까).
 *     → 한 컬럼(세로 줄) 전체가 의미 있는 변경 묶음. ReviewModal 의 컬럼 헤더
 *       클릭 일괄 토글은 이 패턴 때문에 필요.
 *
 *  2) needsLang true type 의 작은 사이즈 빈칸 = 표기 차이 (진짜 빈칸 아님)
 *     옛 단가표는 "영문기본 X" / "한글기본 Y" 텍스트가 base group 의 첫 셀에만
 *     있고 나머지 사이즈는 빈칸으로 둔다. parseChannel 의 baseGroup 로직이
 *     이 텍스트를 해석해 base 사이즈에 가격을 채우지만, 영/한 base 범위가
 *     baseline 에서 다른 경우(예: galvaOsai eng=[200,250], kor=[200,250,300])
 *     일부 사이즈가 누락된다.
 *     → `backfillBaseGroup` + `backfillSmallMissing` 으로 작은 사이즈 누락을
 *       baseline 값으로 자동 채움. 사용자에게 "사이즈 제거" 로 표시 안 됨.
 *
 *  3) 채워진 최대 사이즈 이후의 빈칸 = 실제 사이즈 제거
 *     가격 시프트로 진짜 빠진 사이즈. missing_in_excel 로 유지 →
 *     ReviewModal 에서 "사이즈 제거" 태그 + 자동 체크 ON.
 *     buildPricesFromDecisions 가 'excel' 결정 시 baseline 에서 키 삭제.
 *
 *  4) 잔넬 시트 = 잔넬만 비교 (시트 격리)
 *     사용자가 한 시트만 골랐으면 다른 카테고리(고무·아크릴·...) 는 절대
 *     건드리면 안 됨. sheetName 문자열 받으면 `inferCategoryFromSheetName`
 *     으로 카테고리 추정해서 그 파서만 호출. computeDiff 도 parsed 에 없는
 *     카테고리는 비교 skip.
 */
