 ...
 *  - {@code autoquote.data-dir} 가 존재하지 않는 디렉터리를 가리키고 {@code autoquote.r2-bucket} 도 비어 있으면
...
class AdminAutoQuoteUnavailableTest {
    ...
    /** 존재하지 않는 디렉터리 + 빈 전용 버킷 → 두 소스 모두 미프로비저닝(네트워크 호출 없음). */
    ...
    static void noSourceAvailable(DynamicPropertyRegistry registry) {
        ...
        // 전용 비공개 버킷 프로퍼티를 명시적으로 비워 R2 소스 비활성(네트워크 호출 없음).
        // (공유 r2.bucket 은 컨트롤러가 더 이상 읽지 않으므로 건드리지 않는다.)
